...
pub fn create_sumary_file(
    patterns_by_kind: &HashMap<String, Vec<PatternInfo>>,
    patterns_out: &Path,
) -> Result<()> {
    ...
    if let Err(e) = fs::create_dir_all(out_path) {
        ...
        if let Err(e) = fs::write(&summary_path, summary_json) {
            eprintln!(
                "\x1b[33m⚠ Warning:\x1b[0m Could not write patterns summary: {}",
                e
            );
        } else {
...
pub fn create_grouped_by_kind_file(
    patterns_by_kind: &HashMap<String, Vec<PatternInfo>>,
    patterns_out: &Path,
) -> Result<()> {
    // aggregate by kind
    ...
    if let Err(e) = fs::write(&agg_path, agg_json) {
        eprintln!(
            "\x1b[33m⚠ Warning:\x1b[0m Could not write grouped patterns file: {}",
            e
        );
    } else {
