...
use crate::extractor_utils::{create_grouped_by_kind_file, create_sumary_file};
...
impl<'ast> Visit<'ast> for UnsafeCollector {
    fn visit_expr_unsafe(&mut self, node: &'ast ExprUnsafe) {
        ...
        self.blocks
            .push((node.to_token_stream(), node.unsafe_token.span()));
...
pub fn process_file(path: &Path, out_dir: &Path) -> Result<Vec<pattern_detector::PatternInfo>> {
    ...
    let ast = match syn::parse_file(&src) {
        ...
        Err(e) => {
            eprintln!(
                "\x1b[91m warning!:\x1b[0m skipping {} \x1b[91m(parse error: {})\x1b[0m",
                path.display(),
                e
            );
            return Ok(Vec::new());
    ...
    detector.save_to(out_dir)?; // optional: save per-file patterns in JSON
    detector.save_html(out_dir, path)?; // save HTML representation
